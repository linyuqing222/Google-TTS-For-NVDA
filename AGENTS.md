...
- Standalone regression test code map:
  ...
  **Cross-platform note:** `SafeJoinTests` assertions use `.resolve()` on both expected and actual paths because `_safe_join()` calls `Path.resolve()` internally, which on Windows may return 8.3 short names (e.g. `RUNNER~1` instead of `runneradmin`). Always compare resolved paths when testing `_safe_join` to ensure cross-platform consistency.
...
- Browser harness audio code map:
  ...
  - Synth-to-browser fixed gain: `_OUTPUT_GAIN_MAKEUP` (1.70 in both `__init__.py` and `standby.py`, clamped in `bridgeHarness.js` `outputGainFromPayload()`), `SynthDriver._speech_options()`, and the `outputGain` payload field in `WasmTtsEngineBridge.speak()`.
...
The CI workflow (`.github/workflows/test.yml`) runs on push to main/master and pull requests:

1. **Ruff check**: linting with `ruff check .`
2. **Ruff format check**: formatting with `ruff format --check .`
3. **Mypy type check**: type checking with `mypy --config-file mypy.ini`
...
python -m mypy --config-file mypy.ini --explicit-package-bases --exclude "websocketClientRepo" googleTtsForNvda/synthDrivers/ tests/
