...
- Speech flow integration lives in `speak()`, `_iter_speech_chunks()` (handles `CharacterModeCommand` with inter-character spacing for spelling mode, `PhonemeCommand` with fallback text since browser TTS lacks IPA support), `_sentence_break_milliseconds()`, `_speak_worker()`, `_speak_text()`, `_speak_text().on_segment_end()`, `_short_cache_key()`, and `_finish_request_audio()` (synchronous `player.idle()` when no queued speech, `synthDoneSpeaking` notified immediately after).
