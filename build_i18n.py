    ...
    if requested_languages:
        ...
        for language in requested_languages:
            ...
            if not language_dir.is_dir():
                errors.append(
                    f"{normalized_language}: translation folder is missing: {language_dir.relative_to(ADDON_DIR)}"
                )
                continue
...
def main() -> int:
    ...
    source_messages = (
        _translatable_source_messages() if CHECK_UI in checks or CHECK_OBSOLETE in checks or not check_only else None
    )
